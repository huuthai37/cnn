...
data_folder = '/mnt/smalldata/opt2/train/'
...
if train:
    ...
    result_model.load_weights('mobilenet_opt2_weights_2e.h5')
    ...
    for e in range(epoch):
        ...
        result_model.fit_generator(getTrainData(keys,batch_size,classes), verbose=1, max_queue_size=2, steps_per_epoch=len_samples/batch_size, epochs=1)
        result_model.save_weights('mobilenet_opt2_weights_3e.h5')

    # result_model.save_weights('mobilenet_opt_weights_66e.h5')
else:
    result_model.load_weights('mobilenet_opt2_weights_3e.h5')
    epoch = 1;
    with open(out_file,'rb') as f2:
        keys = pickle.load(f2)
    len_samples = len(keys)
