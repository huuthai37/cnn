...
if train & (not retrain):
    temporal_model.get_layer('conv_new').set_weights(gd.convert_weights(layers[2].get_weights(), depth))
...
if train:
    if retrain:
        result_model.load_weights('weights/mobilenet_twostream1_io_{}e.h5'.format(old_epochs))
    ...
    for e in range(epochs):
        ...
        random.shuffle(keys)
        result_model.fit_generator(gd.getTrainData(keys,batch_size,classes,4,train), verbose=1, max_queue_size=2, steps_per_epoch=len_samples/batch_size, epochs=1)
        result_model.save_weights('weights/mobilenet_twostream1_io_{}e.h5'.format(old_epochs+1+e))

else:
    result_model.load_weights('weights/mobilenet_twostream1_io_{}e.h5'.format(epochs))
