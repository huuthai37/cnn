...
from keras.layers import Dense, Conv2D, Activation, Reshape, Flatten, Input, ZeroPadding2D, Multiply
...
if train:
    if retrain:
        result_model.load_weights('weights/mobilenet_twostream2_mul_{}e.h5'.format(old_epochs))
    ...
    for e in range(epochs):
        ...
        result_model.save_weights('weights/mobilenet_twostream2_mul_{}e.h5'.format(old_epochs+1+e))
    print histories
    with open('data/trainHistoryTwoStreamMul2{}_{}_{}e'.format(2, old_epochs, epochs), 'wb') as file_pi:
        pickle.dump(histories, file_pi)

else:
    result_model.load_weights('weights/mobilenet_twostream2_mul_{}e.h5'.format(epochs))
